...
class PickupSwapViewController: UIViewController,UITextFieldDelegate {
    ...
    let menubtn = UI3()
    let heading = UI3()
    let notificationbtn = UI3()
    //for content
    let body = UI3()
    let one = UI3()
    let onelabel = UI3()
    let check = UI3(), datepicker = UI3(),daypicker = UI3()
    let day = [UI3(),UI3(),UI3(),UI3(),UI3(),UI3(),UI3()]
    let dayname = ["M","T","W","T","F","S","S"]
    var daynameSelect = [false,false,false,false,false,false,false]
    let line1 = UI3()
    var department = UI3(), departmentlabel = UI3(), departmentScroll = UI3(), departmentlist:[UI3] = [], departments:[String] = [],departmentselected = UI3(), departmentselectedlabel:[UI3] = [], departmentselectedbutton:[UI3] = []
    let line2 = UI3()
    let three = UI3()
    let threelabel = UI3()
    let two = UI3()
    let twolabel = UI3()
    let starttime = UI3(), starttimeTrue = UI3(), starttimeFalse = UI3(), starttimeTruelabel = [UI3(),UI3(),UI3(),UI3(),UI3()], starttimeFalselabel = [UI3(),UI3()]
    let starttimepicker = UI3(), starttimelabel = UI3(), starttimebutton = [UI3(),UI3()]
    var starttimeRadios:[Int:Bool] = [1:false,2:false,3:false,4:false]
    let startRange = [UI3(),UI3()], startRangeLabel = [UI3(),UI3(),UI3(),UI3()]
    let endtime = UI3(), endtimeTrue = UI3(), endtimeFalse = UI3(), endtimeTruelabel = [UI3(),UI3(),UI3(),UI3(),UI3()], endtimeFalselabel = [UI3(),UI3()]
    let endRange = [UI3(),UI3()], endRangeLabel = [UI3(), UI3(),UI3(),UI3()]
    ...
    override func viewDidLoad() {
        ...
        checkboxonClick3.function = selectDate_or_Day_Change
        ...
        deleteShift.View(x: 30, y: endtimeFalse.view.frame.maxY+10, width: x-60, height: 50, bkcolor: #colorLiteral(red: 0.501960814, green: 0.501960814, blue: 0.501960814, alpha: 1), cornerRadius: 20, border: 0, borderColor: .clear, view: body.scrollview)
        deleteShiftlable[0].Label(x: 10, y: 0, width: 230, height: 50, txt: "Delete This Shift Request After", fontsize: 16, bold: false, cornerRadius: 0, border: 0, borderColor: UIColor.clear.cgColor, alignment: .center, bkcolor: UIColor.clear, txtcolor: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), view: deleteShift.view)
        deleteShift.Textfield(x: deleteShift.view.frame.maxX-145, y: 0, width: 50, height: 50, placeholder: "", border: 0, borderRadius: 0, txtAlign: .center, bordercolor: UIColor.clear.cgColor, keyboard: .numberPad, textColor: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), bkcolor: UIColor.clear.cgColor, view: deleteShift.view)
        ...
        deleteShiftlable[1].Label(x: deleteShift.view.frame.maxX-90, y: 0, width: 60, height: 50, txt: "Days(s)", fontsize: 16, bold: false, cornerRadius: 0, border: 0, borderColor: UIColor.clear.cgColor, alignment: .center, bkcolor: UIColor.clear, txtcolor: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), view: deleteShift.view)
        deleteShift.txtfield.text = "30"
        deleteShiftlable[0].View(x: deleteShift.txtfield.frame.minX, y: deleteShift.txtfield.frame.maxY-10, width: deleteShift.txtfield.frame.size.width, height: 2, bkcolor: #colorLiteral(red: 0.4666666687, green: 0.7647058964, blue: 0.2666666806, alpha: 1), cornerRadius: 0, border: 0, borderColor: .clear, view: deleteShift.view)
        
        line3.View(x: 30, y: deleteShift.view.frame.maxY+10, width: x-60, height: 1, bkcolor: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), cornerRadius: 0, border: 0, borderColor: .clear, view: body.scrollview)
        ...
        tapRecognizer.addTarget(self, action: #selector(TaponView(_:)))
...
//        self.navigationController?.popViewController(animated: true)
    ...
    @objc func continueButton(_ btn:UIButton){
        ...
        print("-----------")
               print(checkboxonClick3.isChecked) // bool
               ...
               print(deleteShift.txtfield.text!)
               print("-----------")
    ...
    func validation() -> Bool {
        ...
        var toastmessage = ""
        resetPickupShiftvar()
        if checkboxonClick3.isChecked == true {
            PickupShiftvar.Static.permanent = checkboxonClick3.isChecked
            var selected = false
            var index = 0
        ...
        else if checkboxonClick3.isChecked == false {
            PickupShiftvar.Static.permanent = checkboxonClick3.isChecked
        ...
        if deleteShift.txtfield.text! == "" {deleteShift.txtfield.text = "30"}
        else{PickupShiftvar.Static.deleteShiftdays = Int(deleteShift.txtfield.text!)!}
    ...
    func onClickLayoutUpdater() {
        ...
        if endtimeFalse.view.isHidden == false {
            deleteShift.view.frame.origin.y = endtimeFalse.view.frame.maxY+10
        }
        else if endtimeTrue.view.isHidden == false {
            deleteShift.view.frame.origin.y = endtimeTrue.view.frame.maxY+10
        }
        else{
            deleteShift.view.frame.origin.y = endRange[0].view.frame.maxY+10
        }
        
        line3.view.frame.origin.y = deleteShift.view.frame.maxY+10
        body.scrollview.contentSize.height = line3.view.frame.maxY+10
    }
...
class checkboxonClick3 {
    static var function:()->() = {}
    static var isChecked:Bool = false
}
class radiobuttonClick3 {
...
class UI3: UIView {
    ...
    ////////////////////////////////////////////////
    // dependent on class 'checkboxonClick3'
    ...
    @objc func checkboxClick(_btn:UIButton){
        if checkboxonClick3.isChecked == true {
            ...
            checkbox.setBackgroundImage(UIImage(named: "checkboxTrue"), for: .normal)
            checkboxonClick3.isChecked=true
        }
        checkboxonClick3.function()
    }
...
//                        datePicker.locale = Locale(identifier: "en_GB")
    ...
    ///////////////////////////////////
    func AutoSizeLabel(x:CGFloat,y:CGFloat,height:CGFloat,txt:String,fontsize:CGFloat,bkcolor:UIColor,txtcolor:UIColor,align:NSTextAlignment,cornerRadius:CGFloat,view:UIView) {
        ...
        autoSizelabel.textAlignment = align
